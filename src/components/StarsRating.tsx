...
function StarsRating({ rating, imgStarClassName = '', ratingClassName = '' }: StarsProps): JSX.Element {
    ...
    for (let i = 0; i < fullStars; i++) {
        starIcons.push(<img key={i} className={imgStarClassName} src={starIcon} alt="star" />);
    }

    // Menambahkan bintang setengah jika ada
    if (hasHalfStar) {
        starIcons.push(<img key="half" className={imgStarClassName} src={starPartialIcon} alt="half star" />);
    }

    // Mengisi bintang kosong hingga jumlah total menjadi lima
    const remainingStars = 5 - starIcons.length;
    for (let i = 0; i < remainingStars; i++) {
        starIcons.push(<img key={`empty-${i}`} className={imgStarClassName} src={starWhiteIcon} alt="empty star" />);
    }
